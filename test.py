...
def enhance_ct_image_sharpness(image_tensor, intensity=1.0):

    original_device = image_tensor.device
    image_tensor = torch.clamp(image_tensor, 0, 1)
    image_np = image_tensor.squeeze().cpu().numpy()
    image_std = np.std(image_np)
    adaptive_sigma = max(0.8, min(1.2, 1.0 + (image_std - 0.15) * 0.5))
    blurred = cv2.GaussianBlur(image_np, (0, 0), sigmaX=adaptive_sigma, sigmaY=adaptive_sigma)
    high_freq = image_np - blurred
    high_freq_std = np.std(high_freq)
    max_enhancement = min(intensity, 1.0 + high_freq_std * 2.0)
    enhanced = image_np + max_enhancement * high_freq
    if intensity > 1.2:

        enhanced = cv2.bilateralFilter(enhanced.astype(np.float32), d=5, sigmaColor=0.1, sigmaSpace=1.0)
    
    enhanced = np.clip(enhanced, 0, 1)
    return torch.from_numpy(enhanced).unsqueeze(0).unsqueeze(0).to(original_device)
...
def sample_posterior_ddim(coefficients, x_0, x_t, t, eta=0.0):
    ...
    def p_sample_ddim(x_0, x_t, t, eta=0.0):
        mean, _, log_var = q_posterior(x_0, x_t, t)
        
        if eta > 0:
            noise = torch.randn_like(x_t)
            nonzero_mask = (1 - (t == 0).type(torch.float32))
            return mean + nonzero_mask[:,None,None,None] * torch.exp(0.5 * log_var) * noise * eta
        else:
           
            return mean
            
    sample_x_pos = p_sample_ddim(x_0, x_t, t, eta)
    return sample_x_pos
...
def sample_from_model_ddim(coefficients, generator, n_time, x_init, T, opt, eta=0.0):
...
def sample_and_test(args):
    ...
    for iteration, (x , y) in enumerate(tqdm(data_loader)): 
        ...
        #diffusion steps
        if args.use_ddim:
            fake_sample2 = sample_from_model_ddim(pos_coeff, gen_diffusive_2, args.num_timesteps, x2_t, T, args, eta=args.ddim_eta)
        else:
        ...
        if args.use_sharpening:
            fake_sample2 = enhance_ct_image_sharpness(fake_sample2, 
                                                     intensity=args.sharpening_intensity)
        ...
        result.append(fake_sample2.detach().cpu().numpy())
    ...
    result_raw = np.stack(result, axis=0).reshape(-1, 256, 256)
    print(result_raw.shape)

    result_raw.tofile('{}/sCT.raw'.format(save_dir))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_ddim', action='store_true', default=False,
                        help='whether to use DDIM sampling strategy')
    parser.add_argument('--ddim_eta', type=float, default=0.0,
                        help='DDIM eta parameter: 0=deterministic, 1=stochastic, 0<eta<1=intermediate')
    

    parser.add_argument('--use_sharpening', action='store_true', default=False,
                        help='whether to apply sharpening post-processing (unsharp mask)')
    parser.add_argument('--sharpening_intensity', type=float, default=1.0,
                        help='sharpening intensity (0.5-2.0, higher values = stronger sharpening)')
