...
from backbones.losses import GlobalNCC2D, zecon_loss
# from backbones.utils import sobel_filter
...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            # hpf = sobel_filter(source, sobel_eta)
...
def train_syndiff(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            # sobel_eta = np.random.rand()
            # sobel_eta = 0.01 + (0.025 - 0.01) * sobel_eta
            ...
            # x1_sobel = sobel_filter(real_data1, sobel_eta)
            # x2_sobel = sobel_filter(real_data2, sobel_eta)

            # np.save(os.path.join(args.output_path, "debug", "x1_sobel.npy"), x1_sobel.cpu().detach().numpy())
            # np.save(os.path.join(args.output_path, "debug", "x2_sobel.npy"), x2_sobel.cpu().detach().numpy())
            ...
            errG_zecon = zecon_loss(gen_diffusive_1,  x1_0_predict_diff[:,[0],:], x2_0_predict, ref=x2_0_predict, t=t1, z=latent_z1) + \
                            zecon_loss(gen_diffusive_2,  x2_0_predict_diff[:,[0],:], x1_0_predict, ref=x1_0_predict, t=t2, z=latent_z2)

            errG_struct = mse_loss(x1_0_predict_diff[:,[0],:], x2_0_predict) + mse_loss(x2_0_predict_diff[:,[0],:], x1_0_predict)

            errG_adv = errG1 + errG2 + 0.*errG_struct + 0.*errG_zecon
            ...
            #struct loss
            struct_loss_2to1 = mse_loss(x1_0_predict, real_data2)
            struct_loss_1to2 = mse_loss(x2_0_predict, real_data1)
            err_struct = struct_loss_1to2 + struct_loss_2to1
            #zecon loss
            err_zecon = torch.tensor(0.0, device=real_data1.device)

            errG_cycle = errG1_cycle + errG2_cycle + 0.*err_struct + 0.*err_zecon            
            ...
            if iteration % 100 == 0:
                if rank == 0:
                    with open("train.log", "a") as f:
                        print('epoch {} iteration{}, G-Cycle: {}, G-L1: {}, G-Adv: {}, G-cycle-Adv: {}, G-Sum: {}, D Loss: {}, D_cycle Loss: {}'.format(epoch,iteration, errG_cycle.item(), errG_L1.item(),  errG_adv.item(), errG_cycle_adv.item(), errG.item(), errD.item(), errD_cycle.item()), file=f)
                        f.close()
